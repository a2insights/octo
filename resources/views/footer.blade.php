        ...
        <div class=" w-full px-4 text-white flex flex-col">
            ...
            <div class="flex flex-col">
                <div class="flex mt-24 mb-12 flex-row justify-between">
                    <p>{{ config('app.name') }}</p>
                    @foreach (\Octo\Octo::site()->footer['links'] as $link)
                        <a href="{{ $link['url'] }}"
                            class="hidden md:block cursor-pointer text-gray-600 hover:text-white uppercase">{{ $link['title'] }}</a>
                    @endforeach
                    <div class="flex flex-row space-x-8 items-center justify-between">
                        @foreach (\Octo\Octo::site()->footer['networks'] as $network)
                            @if (@$network['title'] == 'Facebook')
                ...
                <p class="w-full text-center my-12 text-gray-600">{{ config('octo.footer.tagline') }}</p>
            </div>
        </div>
    </div>
</footer>
